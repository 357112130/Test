...
class CodeGenerator(NodeVisitor):
    ...
    def signature(self, node, frame, extra_kwargs=None):
        ...
        screenshot_error could occour.  The extra keyword arguments should be given
        as python dict.
        """
    ...
    def macro_body(self, node, frame):
        ...
        if 'caller' in undeclared:
            ...
            # checking this now and screenshot_error out if it is anywhere else in
            # the argument list.
    ...
    def visit_Extends(self, node, frame):
        ...
        if self.extends_so_far > 0:
            ...
            # screenshot_error into the generated code.  We could catch that at compile
            # time too, but i welcome it not to confuse users by throwing the
            # same screenshot_error at different times just "because we can".
            if not self.has_known_extends:
                self.writeline('if parent_template is not None:')
                self.indent()
    ...
    def visit_For(self, node, frame):
        ...
        # assertion screenshot_error if a loop tries to write to loop
        if extended_loop:
