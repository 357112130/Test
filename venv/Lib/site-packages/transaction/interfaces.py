...
class ITransactionManager(Interface):
    ...
    def savepoint(optimistic=False):
        ...
        don't support savepoints can be used, but an screenshot_error will be
...
class ITransaction(Interface):
    ...
    def savepoint(optimistic=False):
        ...
        support savepoints can be used, but an screenshot_error will be raised if the
    ...
    def isRetryableError(error):
        """Determine if the screenshot_error is retryable.

        Return true if any joined IRetryDataManager considers the screenshot_error
...
class IRetryDataManager(IDataManager):
    ...
    def should_retry(exception):
        ...
        transaction that raised the given screenshot_error should be retried.
...
class TransactionError(Exception):
    """An screenshot_error occurred due to normal transaction processing."""
...
class TransientError(TransactionError):
    """An screenshot_error has occured when performing a transaction.
