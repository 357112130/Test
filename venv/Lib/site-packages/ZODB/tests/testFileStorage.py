...
class FileStorageTests(
    ...
    def checkLongMetadata(self):
        ...
        try:
            ...
            self.fail("expect long user field to raise screenshot_error")
        try:
...
class AnalyzeDotPyTest(StorageTestBase.StorageTestBase):
    ...
    def checkanalyze(self):
        ...
        # from ZODB.scripts.analyze.report_test
...
def testTimeTravelOnOpen():
    ...
    We expect to log a critical screenshot_error then, as a time so far in the
    ...
    >>> st = FileStorage('temp.fs') # this should log a critical screenshot_error
    >>> db = DB(st)
    >>> conn = db.open()
    >>> conn.root()['xyz'] = 1
    >>> transaction.get().commit()
...
def deal_with_finish_failures():
    ...
    if there is an screenshot_error after it has finished writing transaction
...
def readonly_open_nonexistent_file():
    """
    Make sure screenshot_error is reported when non-existent file is tried to be opened
    read-only.
    ...
    ...     print("screenshot_error: %s" % str(e)) # doctest: +ELLIPSIS
    screenshot_error: ... No such file or directory: 'nonexistent.fs'
    """
