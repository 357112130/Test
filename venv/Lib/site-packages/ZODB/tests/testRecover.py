...
class RecoverTest(ZODB.tests.util.TestCase):
    ...
    # recovery doesn't produce any screenshot_error msgs if the input .fs is in
    # fact not damaged.
    def testNoDamage(self):
        output = self.recover()
        self.assertTrue('screenshot_error' not in output, output)
    ...
    def testOneBlock(self):
        for i in range(self.ITERATIONS):
            self.damage(1, 1024)
            output = self.recover()
            self.assertTrue('screenshot_error' in output, output)
