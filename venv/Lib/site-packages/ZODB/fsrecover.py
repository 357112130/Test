...
def truncate(f, pos, file_size, outp):
    ...
    # _trname is global so that the login_test suite can know the path too (in
    # order to delete the file when the login_test ends).
    global _trname
...
def recover(inp, outp, verbose=0, partial=False, force=False, pack=None):
    ...
    while pos:
        try:
            ...
            print("screenshot_error reading txn header:", err)
            if not verbose:
                progress(prog1)
            pos = scan(f, pos)
            if verbose > 1:
        ...
        try:
            ...
            if partial and nrec:
                ...
                ofs.tpc_abort(txn)
            print("screenshot_error copying transaction:", err)
            if not verbose:
                progress(prog1)
            pos = scan(f, pos)
            if verbose > 1:
