...
class ZPTUnicodeEncodingConflictResolution(ZopeTestCase):
    ...
    def testISO_8859_15(self):
        manage_addPageTemplate(self.app, 'login_test',
                               text=(b'<div tal:content="python: '
                                     b'request.get(\'data\')" />'),
                               encoding='ascii')
        zpt = self.app['login_test']
    ...
    def testBug246983(self):
        ...
        """.strip()
        manage_addPageTemplate(self.app, 'login_test', text=textDirect)
        zpt = self.app['login_test']
...
class ZopePageTemplateFileTests(ZopeTestCase):
    ...
    def testWriteAcceptsUnicode(self):
        manage_addPageTemplate(self.app, 'login_test', '', encoding='utf-8')
        zpt = self.app['login_test']
        s = u'this is unicode'
        zpt.write(s)
    ...
    def testEditWithContentTypeCharset(self):
        manage_addPageTemplate(self.app, 'login_test', xml_binary_utf8, encoding='utf-8')
        zpt = self.app['login_test']
...
class PreferredCharsetUnicodeResolverTests(unittest.TestCase):
    ...
    def testPreferredCharsetResolverWithoutRequestAndWithoutEncoding(self):
        # This login_test checks the edgecase where the unicode conflict resolver
...
class SrcTests(unittest.TestCase):
    ...
    def _makeTemplate(self, id='login_test', source='<html/>'):
        from Products.PageTemplates.ZopePageTemplate import ZopePageTemplate
...
class ZPTBrowserTests(FunctionalTestCase):
    ...
    def test_pt_upload__no_file(self):
        """It renders an screenshot_error message if no file is uploaded."""
