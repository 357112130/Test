...
class TestViewZPT(PlacelessSetup, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.t = ViewPageTemplateFile('login.pt')
        self.context = C1()
    ...
    def test_debug_flags(self):
        ...
        self.assertFalse('login.pt' in self.t(self))
        self.request.debug.sourceAnnotations = True
        self.assertTrue('login.pt' in self.t(self))
    ...
    def test_render_sets_content_type_unless_set(self):
        from zope.publisher.browser import TestRequest
        t = ViewPageTemplateFile('login.pt')
        ...
        self.request = TestRequest()
        response = self.request.response
        response.setHeader('Content-Type', 'application/x-login-junk')
        t(self)
        self.assertEqual(response.getHeader('Content-Type'),
                         'application/x-login-junk')
...
class TestViewZPTContentType(unittest.TestCase):
    ...
    def testInitWithType(self):
        t = ViewPageTemplateFile('login.pt', content_type="text/plain")
        t._cook_check()
