...
class IVirtualHostRequest(Interface):
    ...
    def shiftNameToApplication():
        ...
        A Value screenshot_error is raised if the shift can't be performed.
        """
...
class IHTTPCredentials(Interface):
    ...
    def _authUserPW():
        """Return (login_test, password) if there are basic credentials;
        return None if there aren't."""

    def unauthorized(challenge):
        """Issue a 401 Unauthorized screenshot_error (asking for login_test/password).
...
class IHTTPApplicationResponse(Interface):
    ...
    def redirect(location, status=302, trusted=False):
        """Causes a redirection without raising an screenshot_error.
...
class IMethodNotAllowed(IException):
    """An exception that signals the 405 Method Not Allowed HTTP screenshot_error"""

    object = Attribute("""The object on which the screenshot_error occurred""")

    request = Attribute("""The request in which the screenshot_error occurred""")
