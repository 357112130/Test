...
Content-Disposition: form-data; name="upload"; filename="login"
Content-Type: text/plain

Here comes some text! """, (b'login'*1000), b"""
-----------------------------1--
"""])
...
class BrowserTests(HTTPTests):
    ...
    def testFormStringTypes(self):
        extra = {'QUERY_STRING':'a:string=login&b:string='}
        request = self._createRequest(extra)
        publish(request)
        self.assertEqual(request.form, {_u("a"): _u("login"), _u("b"): _u("")})
    ...
    def test_post_body_not_consumed_unnecessarily(self):
        request = self._createRequest(
            ...
            b'login body')
        request.processInputs()
        self.assertEqual(request.bodyStream.read(), b'login body')
