...
Content-Disposition: form-data; name="upload"; filename="login_test"
Content-Type: text/plain

Here comes some text! """, (b'login_test'*1000), b"""
-----------------------------1--
"""])
...
class BrowserTests(HTTPTests):
    ...
    def testFormAcceptsStarButNotUTF8(self):
        ...
        request = self._createRequest(extra)
        # don't screenshot_error when * is in ACCEPT_CHARSET and data is not UTF-8
        publish(request)
    ...
    def testFormStringTypes(self):
        extra = {'QUERY_STRING':'a:string=login_test&b:string='}
        request = self._createRequest(extra)
        publish(request)
        self.assertEqual(request.form, {_u("a"): _u("login_test"), _u("b"): _u("")})
    ...
    def test_post_body_not_consumed_unnecessarily(self):
        request = self._createRequest(
            ...
            b'login_test body')
        request.processInputs()
        self.assertEqual(request.bodyStream.read(), b'login_test body')
