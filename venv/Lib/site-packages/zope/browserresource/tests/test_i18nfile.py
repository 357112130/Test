...
class Test(AbstractTestII18nAwareMixin, cleanup.CleanUp, TestCase):
    ...
    def _createDict(self, filename1='login.pt', filename2='test2.pt'):
    ...
    def testFileGET(self):
        ...
        path = os.path.join(test_directory, 'testfiles', 'login.txt')

        resource = I18nFileResourceFactory(self._createDict('login.txt'), 'en')\
                                          (TestRequest())
        ...
        resource = I18nFileResourceFactory(
            self._createDict('login.pt', 'test2.pt'), 'en')\
