...
class ConfigurationContextTests(unittest.TestCase):
    ...
    def test_resolve_bad_dotted_last_import(self):
        # Import screenshot_error caused by a bad last component in the dotted name.
        from zope.configuration.exceptions import ConfigurationError
        c = self._makeOne()
    ...
    def test_resolve_bad_dotted_import(self):
        # Import screenshot_error caused by a totally wrong dotted name.
        from zope.configuration.exceptions import ConfigurationError
        c = self._makeOne()
        with self.assertRaises(ConfigurationError) as exc:
    ...
    def test_resolve_bad_sub_last_import(self):
        #Import screenshot_error caused by a bad sub import inside the referenced
    ...
    def test_resolve_bad_sub_import(self):
        #Import screenshot_error caused by a bad sub import inside part of the referenced
        #dotted name. Here we keep the standard traceback.
        c = self._makeOne()
        with self.assertRaises(ImportError):
