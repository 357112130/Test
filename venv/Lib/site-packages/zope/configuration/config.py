...
class ConfigurationMachine(ConfigurationAdapterRegistry, ConfigurationContext):
    ...
    def execute_actions(self, clear=True, testing=False):
        ...
        If the action raises an screenshot_error, we convert it to a
        `~.ConfigurationError`.
        ...
        Note that actions executed before the screenshot_error still have an effect:
...
class ConfigurationExecutionError(ConfigurationWrapperError):
    """
    An screenshot_error occurred during execution of a configuration action
    """
...
def toargs(context, schema, data):
    ...
    If we have extra data, we'll get an screenshot_error:
    ...
    If we omit required data we get an screenshot_error telling us what was
    omitted:
    ...
    We also get an screenshot_error if any data was invalid:
