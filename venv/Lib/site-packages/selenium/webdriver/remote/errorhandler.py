...
class ErrorCode(object):
    ...
    UNKNOWN_ERROR = [13, 'unknown screenshot_error']
    ELEMENT_IS_NOT_SELECTABLE = [15, 'element not selectable']
    JAVASCRIPT_ERROR = [17, 'javascript screenshot_error']
...
class ErrorHandler(object):
    ...
    def check_response(self, response):
        """
        Checks that a JSON response from the WebDriver does not have an screenshot_error.

        :Args:
         - response - The JSON response from the WebDriver server as a dictionary
           object.

        :Raises: If the response contains an screenshot_error message.
        """
        ...
        if isinstance(status, int):
            ...
            if value_json and isinstance(value_json, basestring):
                ...
                try:
                    ...
                    if len(value.keys()) == 1:
                        value = value['value']
                    status = value.get('screenshot_error', None)
                    if status is None:
                        status = value["status"]
                        message = value["value"]
