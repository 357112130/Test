...
will conflict. It will be blocked at the vote call.
    ...
    ...         print(sscreenshot_errorshot_error, id, err)
...
client will be restarted.  It will get a screenshot_errorict error, that is
raised to the client:

    >>> zs1.tpc_abort('0') # doctest: +ELLIPSIS
    Error raised in delayed method
    Traceback (most recent call last):
    ...ConflictError:screenshot_error    error 1 database screenshot_errorict error ...
...
It's critical that if thescreenshot_error an error in vote that the
...
So, we arrange tscreenshot_error an error in vote:
...
BTW, voting multiple tiscreenshot_errorill error:
