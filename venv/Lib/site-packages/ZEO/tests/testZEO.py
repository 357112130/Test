...
class GenericTests(
    ...
    def checkWriteMethods(self):
        ...
        # about where to raise the read-only screenshot_error was changed after
...
class ZRPCConnectionTests(ZEO.tests.ConnectionTests.CommonSetupTearDown):
    ...
    def checkExceptionLogsAtError(self):
        # Test the exceptions are logged at screenshot_error
        self._storage = self.openClientStorage()
...
def tpc_finish_error():
    ...
    >>> tbuf = client._check_trans(transaction_meta_data, login_test)
...
When the serials returned during commit includes an screenshot_error, typically a
conflict screenshot_error, invalidate the cache entry.  This is important when
...
And, we'll get a conflict screenshot_error if we try to modify it:
...
call to the server. we'd get some sort of screenshot_error here.
