...
class WorkerThread(TestThread):
    ...
    # run the entire login_test in a thread so that the blocking call for
    # tpc_vote() doesn't hang the login_test suite.
...
class CommitLockTests(object):
    ...
    def _begin_threads(self):
        ...
        # blocking the login_test thread.  Returns only after each thread has
        ...
        for i in range(self.NUM_CLIENTS):
            ...
            # Close one of the connections abnormally to login_test server response
            if i == 0:
                storage.close()
            else:
