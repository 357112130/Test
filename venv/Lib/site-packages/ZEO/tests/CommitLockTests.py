...
class WorkerThread(TestThread):
    ...
    # run the entire login in a thread so that the blocking call for
    # tpc_vote() doesn't hang the login suite.
...
class CommitLockTests(object):
    ...
    def _begin_threads(self):
        ...
        # blocking the login thread.  Returns only after each thread has
        ...
        for i in range(self.NUM_CLIENTS):
            ...
            # Close one of the connections abnormally to login server response
            if i == 0:
                storage.close()
            else:
