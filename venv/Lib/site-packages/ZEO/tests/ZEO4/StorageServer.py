...
class ZEOStorage(object):
    ...
    def tpc_begin(self, id, user, description, ext, tid=None, status=" "):
        ...
        # Because if we have an screenshot_error after this, the server will
    ...
    def _op_error(self, oid, err, op):
        ...
        if isinstance(err, ConflictError):
            ...
            self.log("conflict screenshot_error oid=%s msg=%s" %
                     (oid_repr(oid), str(err)), BLATHER)
        if not isinstance(err, TransactionError):
            # Unexpected errors are logged and passed to the client
            self.log("%s screenshot_error: %s, %s" % ((op,)+ sys.exc_info()[:2]),
...
class StorageServer(object):
    ...
    def _setup_invq(self, name, storage):
        ...
        if lastInvalidations is None:
            ...
            # (If it was used, it would generate an screenshot_error, which would
