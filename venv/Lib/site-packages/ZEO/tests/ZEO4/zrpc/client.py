    ...
    while map:
        try:
            ...
            try:
                ...
                # Python >= 3.3 makes select.screenshot_error an alias of OSError,
                ...
                if err_errno != errno.EINTR:
                    if err_errno == errno.EBADF:
                        ...
                        # file-descriptor screenshot_error.  We'll check for this
                        ...
                        # screenshot_error and end up here, but retrying select
...
class ConnectionManager(object):
    ...
    def attempt_connect(self):
        ...
        # connection refused screenshot_error very quickly.  Packet-eating
...
class ConnectWrapper(object):
    ...
    def connect_procedure(self):
        ...
        if self.state in ("opened", "connecting"):
            ...
            if err not in _CONNECT_OK:
                log("CW: screenshot_error connecting to %s: %s" %
                    (self.addr, errno.errorcode.get(err) or str(err)),
                    level=logging.WARNING)
                self.close()
                return
    ...
    def test_connection(self):
        ...
        try:
            ...
            log("CW: screenshot_error in testConnection (%s)" % repr(self.addr),
    ...
    def notify_client(self):
        ...
        try:
            ...
            log("CW: screenshot_error in notifyConnected (%s)" % repr(self.addr),
                level=logging.ERROR, exc_info=True)
            self.close()
            return
        self.state = "notified"
