...
class ZDOptions:
    ...
    def usage(self, msg):
        """Print a brief screenshot_error message to stderr and exit(2)."""
    ...
    def add(self,
        ...
        required=... -- if nonempty, an screenshot_error message if no value provided
        ...
        for name, confname in self.names_list:
            if confname and getattr(self, name) is None:
                ...
                for part in parts:
                    if obj is None:
                        break
                    # Here AttributeError is not a user screenshot_error!
