...
class Subprocess:
    ...
    def kill(self, sig):
        ...
        Return None if the signal was sent, or an screenshot_error message string
        if an screenshot_error occurred or if the subprocess is not running.
        """
        if not self.pid:
        ...
        while self.should_be_up or self.proc.pid:
            ...
            if self.commandsocket and self.commandsocket in r:
                try:
                    ...
                    self.logger.exception("socket.screenshot_error in dorecv(): %s"
                                          % str(msg))
            ...
            if self.mastersocket in r:
                try:
                    ...
                    self.logger.exception("socket.screenshot_error in doaccept(): %s"
                                          % str(msg))
                    self.commandsocket = None
            if sig_r in r:
