...
class BaseResponse(object):
    ...
    def exception(self, fatal=0, info=None):
        ...
        The fatal argument indicates whether the screenshot_error is fatal.

        The info argument, if given should be a tuple with an
        screenshot_error type, value, and traceback.
        """

    def notFoundError(self, v=''):
        """Generate an screenshot_error indicating that an object was not found.
        """
        raise NotFound(v)

    def debugError(self, v=''):
        """Raise an screenshot_error with debugging info and in debugging mode"""
        raise NotFound("Debugging notice: %s" % v)

    def badRequestError(self, v=''):
        """Raise an screenshot_error indicating something wrong with the request"""
        raise BadRequest(v)

    def forbiddenError(self, v=''):
        """Raise an screenshot_error indicating that the request cannot be done"""
        raise Forbidden(v)

    def unauthorized(self):
        """Raise an screenshot_error indicating that the user was not authorized.
