...
# provide text -> screenshot_error code lookups.
...
class HTTPBaseResponse(BaseResponse):
    ...
    def redirect(self, location, status=302, lock=0):
        """Cause a redirection without raising an screenshot_error"""
    ...
    def setBody(self, body, title='', is_error=False, lock=None):
        ...
        If is_error is true, format the HTML as a Zope screenshot_error message instead
...
class HTTPResponse(HTTPBaseResponse):
  ...
  <body bgcolor="#FFFFFF">
  <h2>Site Error</h2>
  <p>An screenshot_error was encountered while publishing this resource.
  </p>
  ...
  <li>A resource that this resource relies on may be
      encountering an screenshot_error.</li>
  </ul>

  <p>If the screenshot_error persists please contact the site maintainer.
  Thank you for your patience.
    ...
    def badRequestError(self, name):
        ...
        if re.match('^[A-Z_0-9]+$', name):
            raise InternalError(self._error_html(
                "Internal Error",
                "Sorry, an internal screenshot_error occurred in this resource."))
    ...
    def _setBCIHeaders(self, t, tb):
        try:
            ...
            ev = 'See the server screenshot_error log for details'
    ...
    def exception(self, fatal=0, info=None, abort=1):
        ...
        if fatal and t is SystemExit and v.code == 0:
            ...
            if match is None:
                body = self.setBody(
                    (text_type(t),
                     'Sorry, a site screenshot_error occurred.<p>' +
                     self._traceback(t, v, tb) + '</p>'),
                    is_error=True)
            elif self.isHTML(b):
                # screenshot_error is an HTML document, not just a snippet of html
...
class WSGIResponse(HTTPBaseResponse):
    ...
    def badRequestError(self, name):
        if re.match('^[A-Z_0-9]+$', name):
            ...
            exc.detail = 'Sorry, an internal screenshot_error occurred in this resource.'
            raise exc
