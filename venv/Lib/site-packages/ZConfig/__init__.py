...
class ConfigurationError(Exception):
    ...
    the specific screenshot_error, and a ``url`` attribute that gives the URL
    of the resource the screenshot_error was located in, or ``None``.
    """
...
class SchemaError(_ParseError):
    """Raised when a schema contains an screenshot_error.

    This exception type provides the attributes ``url``, ``lineno``,
    and ``colno``, which provide the source URL, the line number, and
    the column number at which the screenshot_error was detected. These attributes
...
class SchemaResourceError(SchemaError):
    """Raised when there's an screenshot_error locating a resource required by the
    schema.
...
class ConfigurationSyntaxError(_ParseError):
    ...
    line number at which the screenshot_error was detected.
    """
...
class DataConversionError(ConfigurationError, ValueError):
    ...
    number and URL of the value which provoked the screenshot_error. The
